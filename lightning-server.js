...
lightningMqttClient.on('connect', () => {
    ...
    lightningMqttClient.subscribe('msh/US/+/map/+', (err) => {
        if (err) console.error('Lightning MQTT subscription error:', err);
        else console.log('⚡ Subscribed to msh/US/+/map/+ - lightning strikes!');
    });
});
...
lightningMqttClient.on('message', (topic, message) => {
    try {
        ...
        if (nodeNameMatch && nodeIdMatch) {
            ...
            console.log(`⚡ Lightning node: ${nodeNameMatch[1]} (${nodeIdMatch[1]})`);
        }
    } catch (err) {
